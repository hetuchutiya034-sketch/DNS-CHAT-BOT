import random
import os
from DNSCHAT.database import get_served_chats
...
@DNSCHAT.on_message(filters.command(SHAYRI_COMMAND))
            ...
            [
                [
                    InlineKeyboardButton(
                        "• sᴜᴘᴘᴏꝛᴛ •", url=f"https://t.me/RU_DRA_098"
                    ),
                    InlineKeyboardButton(
                        "• ᴜᴘᴅᴧᴛᴇ •", url=f"https://t.me/RU_DRA_098"
                    ),
                ]
            ]
        ),
...
def get_add_buttons():
    # Built lazily (at send time) instead of once at import time, because
    # DNSCHAT.username is only populated after the client has started
    # (post get_me()). Building this at module import time risked baking
    # in a broken "https://t.me/None?startgroup=true" link forever.
...
async def restart_DNSCHAT():
    os.system(f"kill -9 {os.getpid()} && bash start")


scheduler.add_job(send_good_morning, trigger="cron", hour=6, minute=0)
scheduler.add_job(restart_DNSCHAT, trigger="cron", hour=0, minute=0)
scheduler.add_job(restart_DNSCHAT, trigger="cron", hour=7, minute=0)
scheduler.add_job(restart_DNSCHAT, trigger="cron", hour=12, minute=0)
scheduler.add_job(restart_DNSCHAT, trigger="cron", hour=18, minute=0)
scheduler.start()
